...
const ExamSimulator: React.FC<ExamSimulatorProps> = ({ showHints = false }) => {
  ...
  // Loading/Modal state
  const [isDataLoadModalOpen, setIsDataLoadModalOpen] =
  ...
  useEffect(() => {
    ...
    if (storedProgress) {
      ...
      setQuestions(
        shuffleQuestionsAndOptions(parsedQuestions, progress.isRandomMode),
      );
    } else {
  ...
  const handleSubmitAnswer = () => {
    ...
    if (currentQuestion.isMultiSelect) {
      ...
      pointsToAdd = tempSelectedOptionIds.filter((id) =>
  ...
  return (
    <Container maxWidth="md" sx={{ mt: 4 }}>
      ...
      currentQuestion.type === "dnd-zones" ? (
        <DragAndDropQuestion
      ...
      {showHints && !isFinished && !isDataLoadModalOpen && (
