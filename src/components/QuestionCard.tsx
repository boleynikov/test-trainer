...
import { type AnswerOption, type Question } from "../types";
...
export const QuestionCard: React.FC<QuestionCardProps> = ({
  ...
  const renderOption = (option: AnswerOption) => {
    ...
    return (
      <FormControlLabel
        ...
        sx={(theme) => ({
          border: 1,
          borderColor: ["success", "error"].includes(color)
          ...
          "& .Mui-disabled": {
            color: ["success", "error"].includes(color)
              ? `${theme.palette[color].main} !important`
              : undefined,
          },
        })}
    ...
    >
      <CardContent sx={{ flexGrow: 1 }}>
        <Typography
          variant="h5"
          gutterBottom
          sx={{ fontWeight: "bold", mb: 3, whiteSpace: "pre-line" }}
        >
          {question.text}
        </Typography>

        <Stack spacing={2}>{question.options.map(renderOption)}</Stack>
      </CardContent>
